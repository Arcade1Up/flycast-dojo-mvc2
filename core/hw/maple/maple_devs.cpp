...
struct maple_sega_controller : maple_base {
  ...
  u32 dma(u32 cmd) override {
    ...
    switch (cmd) {
      ...
      case MDC_DeviceReset: return MDRS_DeviceReply;

      case MDC_DeviceKill: return MDRS_DeviceReply;

      default: INFO_LOG(MAPLE, "maple_sega_controller: Unknown maple command %d", cmd);
  ...
  u32 dma(u32 cmd) override {
    ...
    switch (cmd) {
      ...
      case MDCF_GetMediaInfo: {
        u32 function = r32();
        switch (function) {
          case MFID_1_Storage: DEBUG_LOG(MAPLE, "VMU %s GetMediaInfo storage", logical_port);
            w32(MFID_1_Storage);
          ...
          default: INFO_LOG(MAPLE, "VMU: MDCF_GetMediaInfo -> Bad function used |%08X|, returning -2", function);
      ...
      case MDCF_BlockRead: {
        ...
        switch (function) {
          ...
          case MFID_2_LCD: DEBUG_LOG(MAPLE, "VMU %s read LCD", logical_port);
            w32(MFID_2_LCD);
            w32(r32()); // mnn ?
            wptr(flash_data, 192);
          ...
          default: INFO_LOG(MAPLE, "VMU: cmd MDCF_BlockRead -> Bad function |%08X| used, returning -2", function);
            return MDRE_UnknownFunction;//bad function
        }
      }
        break;
      ...
      case MDCF_BlockWrite: {
        ...
        switch (function) {
          ...
          default: INFO_LOG(MAPLE, "VMU: command MDCF_BlockWrite -> Unknown function %x", function);
            return MDRE_UnknownFunction;//bad function
        }
      }
        break;

      case MDCF_GetLastError: return MDRS_DeviceReply;//just ko
      ...
      case MDCF_SetCondition: {
        switch (r32()) {
          ...
          default: INFO_LOG(MAPLE, "VMU: command MDCF_SetCondition -> Bad function used, returning MDRE_UnknownFunction");
            return MDRE_UnknownFunction; //bad function
        }
      }
        break;

      case MDC_DeviceReset: vmuBeep(0, 0);
        return MDRS_DeviceReply;

      case MDC_DeviceKill: vmuBeep(0, 0);
        return MDRS_DeviceReply;

      default: DEBUG_LOG(MAPLE, "Unknown MAPLE COMMAND %d", cmd);
        return MDRE_UnknownCmd;
    }
  }
...
struct maple_microphone : maple_base {
  ...
  u32 dma(u32 cmd) override {
    switch (cmd) {
      case MDC_DeviceRequest:
      case MDC_AllStatusReq: DEBUG_LOG(MAPLE, "maple_microphone::dma MDC_DeviceRequest");
      ...
      case MDC_DeviceReset: DEBUG_LOG(MAPLE, "maple_microphone::dma MDC_DeviceReset");
        if (sampling)
          StopAudioRecording();
        OnSetup();
        return MDRS_DeviceReply;
        ...
        switch (function) {
          case MFID_4_Mic: {
            ...
            switch (subcommand) {
              ...
              case MDRE_TransmitAgain: WARN_LOG(MAPLE, "maple_microphone::dma MDCF_MICControl MDRE_TransmitAgain");
                //apparently this doesnt matter
                //wptr(micdata, SIZE_OF_MIC_DATA);
                return MDRS_DeviceReply;//MDRS_DataTransfer;

              default: INFO_LOG(MAPLE, "maple_microphone::dma UNHANDLED DT1 %02x DT23 %04x", dt1, dt23);
                return MDRE_UnknownFunction;
            }
          }

          default: INFO_LOG(MAPLE, "maple_microphone::dma UNHANDLED function %#010x", function);
            return MDRE_UnknownFunction;
        }
        break;
      }
      ...
      default: INFO_LOG(MAPLE, "maple_microphone::dma UNHANDLED MAPLE COMMAND %d", cmd);
        return MDRE_UnknownCmd;
    }
  }
};
...
struct maple_sega_purupuru : maple_base {
  ...
  u32 dma(u32 cmd) override {
    switch (cmd) {
      ...
      default: INFO_LOG(MAPLE, "UNKOWN MAPLE COMMAND %d", cmd);
        return MDRE_UnknownCmd;
    }
  }
};
    ...
    switch (cmd) {
      ...
      case MDC_AllStatusReq:
        ...
        switch (settings.input.keyboardLangId) {
          case KeyboardLayout::JP: w8(2);    // 92 keys
            break;
          case KeyboardLayout::US: w8(5);    // 104 keys
            break;
          default: w8(6);    // 105 keys
            break;
        }
        w8(0);
      ...
      default: INFO_LOG(MAPLE, "Keyboard: unknown MAPLE COMMAND %d", cmd);
        return MDRE_UnknownCmd;
    }
  }
};
...
struct maple_mouse : maple_base {
  ...
  u32 dma(u32 cmd) override {
    switch (cmd) {
      ...
      default: INFO_LOG(MAPLE, "Mouse: unknown MAPLE COMMAND %d", cmd);
        return MDRE_UnknownCmd;
    }
  }
};
...
struct maple_lightgun : maple_base {
  ...
  u32 dma(u32 cmd) override {
    switch (cmd) {
      ...
      default: INFO_LOG(MAPLE, "Light gun: unknown MAPLE COMMAND %d", cmd);
        return MDRE_UnknownCmd;
    }
  }
...
maple_device *maple_Create(MapleDeviceType type) {
  ...
  switch (type) {
    ...
    case MDT_Microphone: rv = new maple_microphone();
      break;

    case MDT_SegaVMU: rv = new maple_sega_vmu();
      break;

    case MDT_PurupuruPack: rv = new maple_sega_purupuru();
      break;

    case MDT_Keyboard: rv = new maple_keyboard();
      break;

    case MDT_Mouse: rv = new maple_mouse();
      break;
    ...
    case MDT_NaomiJamma: rv = new maple_naomi_jamma();
      break;

    case MDT_TwinStick: rv = new maple_sega_twinstick();
      break;

    case MDT_AsciiStick: rv = new maple_ascii_stick();
      break;

    default: ERROR_LOG(MAPLE, "Invalid device type %d", type);
