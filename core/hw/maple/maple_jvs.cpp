...
class jvs_namco_v226 : public jvs_io_board {
 ...
 protected:
  ...
  u16 read_analog_axis(int player_num, int player_axis, bool inverted) override {
    switch (player_axis) {
      case 0: return read_joystick_x(0);
      case 1: return read_joystick_y(0);
      case 2: return read_joystick_x(1);
      case 3: return read_joystick_y(1);
      case 4: return read_joystick_x(2);
      case 5: return read_joystick_y(2);
      case 6: return read_joystick_x(3);
      case 7: return read_joystick_y(3);
      case 8: return mapleInputState[0].halfAxes[PJTI_R] << 8;
      case 9: return mapleInputState[1].halfAxes[PJTI_R] << 8;
      case 10: return mapleInputState[2].halfAxes[PJTI_R] << 8;
      case 11: return mapleInputState[3].halfAxes[PJTI_R] << 8;
 ...
 protected:
  ...
  u16 read_analog_axis(int player_num, int player_axis, bool inverted) override {
    switch (player_axis) {
      case 0: return read_joystick_x(0);
      case 1: return read_joystick_y(0);
      case 4: return read_joystick_x(1);
      case 5: return read_joystick_y(1);
      default: return 0x8000;
    }
  }
...
maple_naomi_jamma::maple_naomi_jamma() {
  switch (settings.input.JammaSetup) {
    case JVS::Default:
    default: io_boards.push_back(std::make_unique<jvs_837_13551>(1, this));
      break;
    case JVS::FourPlayers: io_boards.push_back(std::make_unique<jvs_837_13551_4P>(1, this));
      break;
    case JVS::RotaryEncoders: io_boards.push_back(std::make_unique<jvs_837_13938>(1, this));
      io_boards.push_back(std::make_unique<jvs_837_13551>(2, this));
      break;
    case JVS::OutTrigger: io_boards.push_back(std::make_unique<jvs_837_13938>(1, this));
      io_boards.push_back(std::make_unique<jvs_837_13551_noanalog>(2, this));
      break;
    case JVS::SegaMarineFishing: io_boards.push_back(std::make_unique<jvs_837_13844>(1, this));
      break;
    case JVS::DualIOBoards4P:
      ...
      break;
    case JVS::LightGun: io_boards.push_back(std::make_unique<jvs_namco_jyu>(1, this));
      break;
    case JVS::LightGunAsAnalog:
      ...
      break;
    case JVS::Mazan: io_boards.push_back(std::make_unique<jvs_namco_fcb>(1, this));
      io_boards.push_back(std::make_unique<jvs_namco_fcb>(2, this));
      break;
    case JVS::GunSurvivor: io_boards.push_back(std::make_unique<jvs_namco_fca>(1, this));
      break;
    case JVS::DogWalking: io_boards.push_back(std::make_unique<jvs_837_13844_encoders>(1, this));
      break;
    case JVS::TouchDeUno: io_boards.push_back(std::make_unique<jvs_837_13844_touch>(1, this));
      break;
    case JVS::WorldKicks: io_boards.push_back(std::make_unique<jvs_namco_v226>(1, this));
      break;
    case JVS::WorldKicksPCB: io_boards.push_back(std::make_unique<jvs_namco_v226_pcb>(1, this));
      break;
    case JVS::WaveRunnerGP: io_boards.push_back(std::make_unique<jvs_837_13844_wrungp>(1, this));
      break;
  }

  if (!ggpo::ggpoLoadingState()) {
    ...
    if (f) {
      if (std::fread(eeprom, 1, 0x80, f) != 0x80)
        ZGS_LOG("Failed or truncated read of EEPROM '%s'", eeprom_file.c_str());
      std::fclose(f);
      ZGS_LOG("Loaded EEPROM from %s", eeprom_file.c_str());
    } else if (naomi_default_eeprom != nullptr) {
      ZGS_LOG("Using default EEPROM file");
      memcpy(eeprom, naomi_default_eeprom, 0x80);
    } else {
      ZGS_LOG("EEPROM file not found at %s and no default found", eeprom_file.c_str());
    }
  } else {
...
void maple_naomi_jamma::handle_86_subcommand() {
  ...
  if (crazy_mode) {
    switch (subcode) {
      case 0x13: subcode = 0x17;
        break;
      case 0x17: subcode = 0x13;
        break;
    }
  }
  u8 node_id = 0;
  ...
  switch (subcode) {
    ...
    case 0x0B: { //EEPROM write
      ...
      ZGS_LOG("EEprom write 0x%x, size: %d", address, size);
      ...
      if (f) {
        ...
        std::fclose(f);
        ZGS_LOG("Saved EEPROM to %s", eeprom_file.c_str());
      } else {
        ZGS_LOG("EEPROM SAVE FAILED to %s", eeprom_file.c_str());
      }
    ...
    case 0x3: { // EEPROM read
      ...
      ZGS_LOG("EEprom READ: 0x%x", address);
      // printState(Command,buffer_in,buffer_in_len);
      sen::emulator.setReceiveKey(true);
      w8(MDRS_JVSReply);
      w8(0x00);
      w8(0x20);
      w8(0x20);
...
u32 maple_naomi_jamma::RawDma(u32 *buffer_in, u32 buffer_in_len, u32 *buffer_out) {
  ...
  switch (cmd) {
    case MDC_JVSSelfTest: w8(MDRS_JVSSelfTestReply);
      w8(0x00);
      w8(0x20);
      w8(0x01);
      w8(0x00);
    ...
    case MDC_JVSCommand: handle_86_subcommand();
      break;
    ...
    case MDC_DeviceRequest: w8(MDRS_DeviceStatus);
      w8(0x00);
      w8(0x20);
      w8(0x00);
      break;
    ...
    case MDC_DeviceKill: w8(MDRS_DeviceReply);
      w8(0x00);
      w8(0x20);
      w8(0x00);
      break;

    case MDCF_GetCondition: w8(MDRE_UnknownCmd);
      w8(0x00);
      w8(0x00);
      w8(0x00);
    ...
    default: INFO_LOG(MAPLE, "Unknown Maple command %x", cmd);
      w8(MDRE_UnknownCmd);
      w8(0x00);
      w8(0x00);
      w8(0x00);
...
u32 jvs_io_board::handle_jvs_message(u8 *buffer_in, u32 length_in, u8 *buffer_out) {
  ...
  switch (jvs_cmd) {
    ...
    case 0x70: LOGJVS("JVS 0x70: %02x %02x %02x %02x", buffer_in[1], buffer_in[2], buffer_in[3], buffer_in[4]);
      ...
      if (jvs_cmd >= 0x20 && jvs_cmd <= 0x38) // Read inputs and more
      {
        ...
        for (u32 cmdi = 0; cmdi < length_in;) {
          switch (buffer_in[cmdi]) {
            ...
            case 0x33: LOGJVS("output(%d) %x", buffer_in[cmdi + 1], buffer_in[cmdi + 2]);
            ...
            default: DEBUG_LOG(MAPLE, "JVS: Unknown input type %x", buffer_in[cmdi]);
