        ...
        if (NaomiGameInputs->axes[axis].name != NULL) {
          if (NaomiGameInputs->axes[axis].type == Full) {
            switch (NaomiGameInputs->axes[axis].axis) {
              case 0: pjs->joy[axis] = GetBtFromSgn(inputState.fullAxes[PJAI_X1]);
                break;
              case 1: pjs->joy[axis] = GetBtFromSgn(inputState.fullAxes[PJAI_Y1]);
                break;
              case 2: pjs->joy[axis] = GetBtFromSgn(inputState.fullAxes[PJAI_X2]);
                break;
              case 3: pjs->joy[axis] = GetBtFromSgn(inputState.fullAxes[PJAI_Y2]);
                break;
              default: pjs->joy[axis] = 0x80;
                break;
            }
          } else {
            switch (NaomiGameInputs->axes[axis].axis) {
              case 4: pjs->joy[axis] = inputState.halfAxes[PJTI_R];
                break;
              case 5: pjs->joy[axis] = inputState.halfAxes[PJTI_L];
                break;
              default: pjs->joy[axis] = 0;
                break;
            }
          }
...
static void createDreamcastDevices() {
  for (int bus = 0; bus < MAPLE_PORTS; ++bus) {
    switch (config::MapleMainDevices[bus]) {
      case MDT_SegaController: mcfg_Create(MDT_SegaController, bus, 5);
      ...
      case MDT_Keyboard: mcfg_Create(MDT_Keyboard, bus, 5);
        break;

      case MDT_Mouse: mcfg_Create(MDT_Mouse, bus, 5);
        break;

      case MDT_LightGun: mcfg_Create(MDT_LightGun, bus, 5);
        if (config::MapleExpansionDevices[bus][0] != MDT_None)
          mcfg_Create(config::MapleExpansionDevices[bus][0], bus, 0);
        break;

      case MDT_TwinStick: mcfg_Create(MDT_TwinStick, bus, 5);
        if (config::MapleExpansionDevices[bus][0] != MDT_None)
          mcfg_Create(config::MapleExpansionDevices[bus][0], bus, 0);
        break;

      case MDT_AsciiStick: mcfg_Create(MDT_AsciiStick, bus, 5);
        if (config::MapleExpansionDevices[bus][0] != MDT_None)
          mcfg_Create(config::MapleExpansionDevices[bus][0], bus, 0);
        break;
      ...
      default: WARN_LOG(MAPLE, "Invalid device type %d for port %d", (MapleDeviceType) config::MapleMainDevices[bus], bus);
        break;
    }
  }
}
...
void mcfg_CreateDevices() {
  switch (settings.platform.system) {
    case DC_PLATFORM_DREAMCAST: createDreamcastDevices();
      break;
    case DC_PLATFORM_NAOMI:
    case DC_PLATFORM_NAOMI2: createNaomiDevices();
      break;
    case DC_PLATFORM_ATOMISWAVE: createAtomiswaveDevices();
      break;
    default: die("Unknown system");
      break;
  }
  vmuDigest();
}
...
void mcfg_SerializeDevices(Serializer &ser) {
  ...
  if (ser.rollback()) {
    // only JVS
    MapleDevices[0][5]->serialize(ser);
  } else {
    for (int i = 0; i < MAPLE_PORTS; i++)
      for (int j = 0; j < 6; j++) {
...
void mcfg_DeserializeDevices(Deserializer &deser) {
  if (deser.rollback()) {
    ...
    // only JVS
    MapleDevices[0][5]->deserialize(deser);
