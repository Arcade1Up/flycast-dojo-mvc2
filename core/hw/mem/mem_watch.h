...
class Watcher {
 ...
 public:
  ...
  void unprotect() {
    ...
    // } else {
    //   for (const auto &pair : pages)
    //     static_cast<T &>(*this).unprotectMem(pair.first, PAGE_SIZE);
    // }
    // pages.clear();
    static_cast<T&>(*this).unprotectMem(0, 0xffffffff);
  }
...
inline static bool writeAccess(void *p) {
  if (!config::GGPOEnable && !kNetVSGame)
    return false;
  if (ramWatcher.hit(p)) {
    bm_RamWriteAccess(p);
    return true;
...
inline static void protect() {
  if (!config::GGPOEnable && !kNetVSGame)
  	return;
  vramWatcher.protect();
  vramWatcher.reserve(40);
  ramWatcher.protect();
  ramWatcher.reserve(160);
  aramWatcher.protect();
  aramWatcher.reserve(5);
//  elanWatcher.protect();
}
