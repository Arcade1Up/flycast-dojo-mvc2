...
T ReadMem_aica_rtc(u32 addr) {
  switch (addr & 0xFF) {
    case 0: return (T) (RealTimeClock >> 16);
    case 4: return (T) (RealTimeClock & 0xFFFF);
    case 8: return 0;
  }
...
void WriteMem_aica_rtc(u32 addr, T data) {
  switch (addr & 0xFF) {
    ...
    case 4:
      ...
      break;
    case 8: rtc_EN = data & 1;
      break;

    default: WARN_LOG(AICA, "WriteMem_aica_rtc: invalid address %x sz %d data %x", addr, (int) sizeof(T), data);
      break;
  }
}
...
T ReadMem_aica_reg(u32 addr) {
  ...
  if (sizeof(T) == 1) {
    switch (addr) {
      case 0x2C00: return (T) ARMRST;
      case 0x2C01: return (T) VREG;
      default: break;
    }
  } else if (addr == 0x2C00)
...
void WriteMem_aica_reg(u32 addr, T data) {
  ...
  if (sizeof(T) == 1) {
    switch (addr) {
      case 0x2C00: ARMRST = data;
        INFO_LOG(AICA_ARM, "ARMRST = %02X", ARMRST);
        ArmSetRST();
        return;
      case 0x2C01: VREG = data;
