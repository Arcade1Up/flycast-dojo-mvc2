...
struct ChannelEx {
  ...
  struct {
    ...
    __forceinline void Step(ChannelEx *ch) {
      counter--;
      if (counter == 0) {
        state++;
        counter = start_value;
...
__forceinline void StepDecodeSample(ChannelEx *ch, u32 CA) {
  ...
  SampleType s0, s1;
  switch (PCMS) {
    case -1: ch->noise_state = ch->noise_state * 16807 + 0xbeef;    //beef is good
...
void CalcAlfo(ChannelEx *ch) {
  u32 rv;
  switch (Type) {
    case LFOType::Sawtooth: rv = ch->lfo.state;
      break;

    case LFOType::Square: rv = ch->lfo.state & 0x80 ? 255 : 0;
      break;

    case LFOType::Triangle: rv = (ch->lfo.state & 0x7f) ^ (ch->lfo.state & 0x80 ? 0x7F : 0);
      rv <<= 1;
      break;
...
void CalcPlfo(ChannelEx *ch) {
  ...
  switch (Type) {
    ...
    case LFOType::Square: rv = ch->lfo.state & 0x80 ? 0xff : 0;
      break;
    ...
    case LFOType::Random: rv = (ch->lfo.state >> 3) ^ (ch->lfo.state << 3) ^ (ch->lfo.state & 0xE3);
      break;
  }
...
void AegStep(ChannelEx *ch) {
  switch (state) {
    case EG_Attack:
      ...
      break;
    case EG_Decay1: ch->AEG.val += ch->AEG.Decay1Rate;
      ...
      break;
    case EG_Decay2: ch->AEG.val += ch->AEG.Decay2Rate;
...
void FegStep(ChannelEx *ch) {
  ...
  u32 target;
  switch (state) {
    case EG_Attack: delta = ch->FEG.AttackRate;
      target = ch->ccd->FLV1;
      break;
    case EG_Decay1: delta = ch->FEG.Decay1Rate;
      target = ch->ccd->FLV2;
      break;
    case EG_Decay2: delta = ch->FEG.Decay2Rate;
      target = ch->ccd->FLV3;
      break;
    case EG_Release: delta = ch->FEG.ReleaseRate;
      target = ch->ccd->FLV4;
      break;
  }
  target <<= EG_STEP_BITS;
