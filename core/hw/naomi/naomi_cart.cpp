...
void naomi_cart_LoadBios(const char *filename) {
  ...
  if (!loadBios(bios, archive.get(), parent_archive.get(), config::Region)) {
    ZGS_LOG("Region %d bios not found in %s", (int) config::Region, bios);
...
static void loadMameRom(const char *filename, LoadProgress *progress) {
  ...
  std::unique_ptr<Archive> archive(OpenArchive(filename));
  if (archive != nullptr)
    ZGS_LOG("Opened NAOMI mame rom %s", filename);

  std::unique_ptr<Archive> parent_archive;
  if (game->parent_name != nullptr) {
    parent_archive.reset(OpenArchive((get_game_dir() + game->parent_name).c_str()));
    if (parent_archive != nullptr)
      ZGS_LOG("Opened NAOMI parent mame rom %s", game->parent_name);
  }
  ...
  try {
    switch (game->cart_type) {
      case M1: CurrentCartridge = new M1Cartridge(game->size);
        break;
      case M2: CurrentCartridge = new M2Cartridge(game->size);
        break;
      case M4: CurrentCartridge = new M4Cartridge(game->size);
        break;
      case AW: CurrentCartridge = new AWCartridge(game->size);
        break;
      case GD: {
        ...
        break;
      default: die("Unsupported cartridge type\n");
        break;
    }
    ...
    if (naomi_default_eeprom == nullptr && game->eeprom_dump != nullptr)
...
static void loadDecryptedRom(const char *file, LoadProgress *progress) {
  ...
  if (!loadBios("naomi", nullptr, nullptr, config::Region)) {
    WARN_LOG(NAOMI, "Warning: Region %d bios not found in naomi.zip", config::Region.get());
    if (!loadBios("naomi", nullptr, nullptr, -1)) {
      if (!bios_loaded)
...
void naomi_cart_UpdateEEPROM() {
  write_naomi_eeprom(0x09, sen::configs.parse_result_["freeplay"].as<bool>() ? 0x1a : 0x00);
  write_naomi_eeprom(0x32, sen::configs.parse_result_["demo-sound-half"].as<int>() - 1);
  write_naomi_eeprom(0x35, sen::configs.parse_result_["difficulty"].as<int>() - 1);
  write_naomi_eeprom(0x36, sen::configs.parse_result_["damage-level"].as<int>() - 1);
  write_naomi_eeprom(0x37, sen::configs.parse_result_["timer-speed"].as<int>() - 1);
  write_naomi_eeprom(0x3a, sen::configs.parse_result_["game-speed"].as<int>() - 1);
}
...
u32 NaomiCartridge::ReadMem(u32 address, u32 size) {
  ...
  switch (address & 255) {
    ...
    case NAOMI_ROM_OFFSETH_addr & 255: return RomPioOffset >> 16 | (RomPioAutoIncrement << 15);

    case NAOMI_ROM_OFFSETL_addr & 255: return RomPioOffset & 0xFFFF;
    ...
    case NAOMI_DMA_COUNT_addr & 255: return (u16) DmaCount;

    case NAOMI_BOARDID_READ_addr & 255: return NaomiGameIDRead() ? 0x8000 : 0x0000;
    ...
    case NAOMI_DMA_OFFSETH_addr & 255: return DmaOffset >> 16;
    case NAOMI_DMA_OFFSETL_addr & 255: return DmaOffset & 0xFFFF;

    case NAOMI_BOARDID_WRITE_addr & 255: DEBUG_LOG(NAOMI, "naomi ReadBoardId: %X, %d", address, size);
      return 1;
...
void NaomiCartridge::WriteMem(u32 address, u32 data, u32 size) {
  switch (address & 255) {
    ...
    case NAOMI_ROM_OFFSETH_addr & 255: RomPioAutoIncrement = (data & 0x8000) != 0;
      RomPioOffset &= 0x0000ffff;
      RomPioOffset |= (data << 16) & 0x7fff0000;
      PioOffsetChanged(RomPioOffset);
      return;

    case NAOMI_ROM_OFFSETL_addr & 255: RomPioOffset &= 0xffff0000;
      RomPioOffset |= data;
      PioOffsetChanged(RomPioOffset);
      return;

    case NAOMI_ROM_DATA_addr & 255: Write(RomPioOffset, size, data);
      if (RomPioAutoIncrement)
        RomPioOffset += 2;

      return;

    case NAOMI_DMA_OFFSETH_addr & 255: DmaOffset &= 0x0000ffff;
      DmaOffset |= (data & 0x7fff) << 16;
      DmaOffsetChanged(DmaOffset);
      return;

    case NAOMI_DMA_OFFSETL_addr & 255: DmaOffset &= 0xffff0000;
      DmaOffset |= data;
      DmaOffsetChanged(DmaOffset);
      return;
    ...
    case NAOMI_DMA_COUNT_addr & 255: {
      ...
      return;
    case NAOMI_BOARDID_WRITE_addr & 255: NaomiGameIDWrite((u16) data);
      return;
    ...
    case NAOMI_BOARDID_READ_addr & 255: DEBUG_LOG(NAOMI, "naomi WriteMem: %X <= %X, %d", address, data, size);
      return;

    case NAOMI_LED_addr & 0xff: return;
...
bool M2Cartridge::Write(u32 offset, u32 size, u32 data) {
  if (offset & 0x40000000) {
    ...
    switch (offset & 0x1ffff) {
      case 0x1fff8: cyptoSetLowAddr(data);
        return true;
      case 0x1fffa: cyptoSetHighAddr(data);
        return true;
      case 0x1fffc: cyptoSetSubkey(data);
        return true;
    }
  }
