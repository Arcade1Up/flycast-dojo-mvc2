...
struct SRamChip : WritableChip {
  ...
  void Write(u32 addr, u32 val, u32 sz) override {
    ...
    if (addr < write_protect_size)
      return;
    switch (sz) {
      case 1: data[addr] = (u8) val;
        return;
      case 2: *(u16 *) &data[addr] = (u16) val;
        return;
      case 4: *(u32 *) &data[addr] = val;
        return;
      default: die("invalid access size");
    }
  }
...
extern void write_naomi_flash(u32 addr, u8 value);
struct NaomiSRamChip : SRamChip {
  NaomiSRamChip(u32 sz, u32 write_protect_size = 0) : SRamChip(sz, write_protect_size) {}
  void Update() const override {
    // write_naomi_flash(0x224, 00);
    // write_naomi_flash(0x230, 00);
    // write_naomi_flash(0x248, 12);
  }
};
...
struct DCFlashChip : WritableChip {
  ...
  void Write(u32 addr, u32 val, u32 sz) override {
    ...
    switch (state) {
        ...
        switch (val & 0xff) {
          ...
          case 0xaa:  // AMD ID select part 1
            if ((addr & 0xfff) == 0x555 || (addr & 0xfff) == 0xaaa)
              state = FS_ReadAMDID1;
            break;
          default: INFO_LOG(FLASHROM, "Unknown FlashWrite mode: %x", val);
            break;
        }
        break;
      ...
      case FS_EraseAMD3:
        ...
        state = FS_Normal;
        break;
      default: WARN_LOG(FLASHROM, "FlashRom: invalid state. write @ %x: %x", addr, val);
        state = FS_Normal;
        break;
    }
  }
  ...
  u8 Read8(u32 addr) override {
    if (state == FS_SelectMode) {
      ...
      switch (addr & 0x43) {
        ...
        default: WARN_LOG(FLASHROM, "SelectMode unknown address %x", addr);
          return 0;
      }
    }
  ...
  void GetPartitionInfo(int part_id, int *offset, int *size) {
    switch (part_id) {
      case FLASH_PT_FACTORY: *offset = 0x1a000;
        *size = 8 * 1024;
        break;
      case FLASH_PT_RESERVED: *offset = 0x18000;
        *size = 8 * 1024;
        break;
      case FLASH_PT_USER: *offset = 0x1c000;
        *size = 16 * 1024;
        break;
      case FLASH_PT_GAME: *offset = 0x10000;
        *size = 32 * 1024;
        break;
      case FLASH_PT_UNKNOWN: *offset = 0x00000;
        *size = 64 * 1024;
        break;
      default: *offset = 0;
        *size = 0;
