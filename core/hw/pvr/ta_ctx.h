...
struct PolyParam
{
  ...
  bool isNaomi2() const { return false; }
};
...
struct TA_context
{
  ...
  void Alloc()
  {
    ...
    rend.verts.Init(38000, &rend.Overrun, "verts");
    rend.idx.Init(60 * 1024, &rend.Overrun, "idx");
    ...
    rend.modtrig.Init(4096, &rend.Overrun, "modtrig");
		
    rend.render_passes.Init(sizeof(RenderPass) * MAX_PASSES, &rend.Overrun, "render_passes");	// 10 render passes
    rend.matrices.Init(1, &rend.Overrun, "matrices");
    rend.lightModels.Init(1, &rend.Overrun, "lightModels");
