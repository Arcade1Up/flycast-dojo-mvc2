...
static u32 DYNACALL read_elanreg(u32 paddr) {
  ...
  switch (addr >> 16) {
    ...
    default: INFO_LOG(PVR, "Read from area2 not implemented [Unassigned], addr=%x", addr);
      return 0;
  }
}
...
static void DYNACALL write_elanreg(u32 paddr, u32 data) {
  ...
  switch (addr >> 16) {
    ...
    default: INFO_LOG(PVR, "Write to area2 not implemented [Unassigned], addr=%x, data=%x", addr, data);
      break;
  }
}
    ...
    if (enabled) {
      ...
      switch (n) {
        case 0:
          // fully clipped
          break;
        case 3: ta_add_triangle(tri);
          break;
        case 4: ta_add_triangle(tri);
...
static void sendPolygon(ICHList *list) {
  ...
  switch (list->flags) {
    ...
    default: WARN_LOG(PVR, "Unhandled poly format %x", list->flags);
      ...
      switch (cmd->pcw.n2Command) {
        ...
        case PCW::registerWait: {
          ...
          if (wait->offset != (u32) -1 && wait->mask != 0) {
            ...
            switch (wait->mask) {
              case 0x80: inter = holly_OPAQUE;
                break;
              case 0x100: inter = holly_OPAQUEMOD;
                break;
              case 0x200: inter = holly_TRANS;
                break;
              case 0x400: inter = holly_TRANSMOD;
                break;
              case 0x200000: inter = holly_PUNCHTHRU;
                break;
              default: WARN_LOG(PVR, "Unknown interrupt mask %x", wait->mask);
        ...
        default: WARN_LOG(PVR, "Unhandled Elan command %x", cmd->pcw.n2Command);
          size -= 32;
          break;
      }
    } else {
      if (Active) {
        ...
        while (i < size) {
          ...
          if (pcw.naomi2 == 1)
            break;
          switch (pcw.paraType) {
            case ParamType_End_Of_List: listType = -1;
              i += 32;
              break;
            case ParamType_Object_List_Set:
            case ParamType_User_Tile_Clip: i += 32;
              break;
            ...
            case ParamType_Sprite:
              ...
              i += 32;
              break;
            case ParamType_Vertex_Parameter: i += vertexSize;
              break;
            default: WARN_LOG(PVR, "Invalid param type %d", pcw.paraType);
              i = size;
              break;
          }
        }
