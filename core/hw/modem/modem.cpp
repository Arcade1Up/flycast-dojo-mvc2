...
#define MODEM_LOG(...) DEBUG_LOG(MODEM, __VA_ARGS__)
...
static int modem_sched_func(int tag, int cycles, int jitter)
{
	...
	case MS_NORMAL:
		...
		case DIALING:
			...
			{
				MODEM_LOG("Switching to RINGING state");
		...
		case CONNECTED:
			...
			if (!mem_dumped)
			{
				...
				for (int i = 0 ; i < sizeof(modem_regs); i++)
					MODEM_LOG("modem_regs %02x == %02x", i, modem_regs.ptr[i]);
			}
#endif
...
static void check_start_handshake()
{
	if (modem_regs.reg09.DTR
			...
			&& connect_state == RINGING)
	{
		MODEM_LOG("DTR asserted. starting handshaking");
...
static void ModemNormalWrite(u32 reg, u32 data)
{
	...
	//	MODEM_LOG("ModemNormalWrite : %03X=%X", reg,data);
	...
	case 0x06:
		MODEM_LOG("PEN = %d", modem_regs.reg06.PEN);
	...
	case 0x08:
		MODEM_LOG("TPDM = %d ASYN = %d", modem_regs.reg08.TPDM, modem_regs.reg08.ASYN);
		break;
	...
	case 0x10:	// TBUFFER
		...
		{
			//MODEM_LOG("ModemNormalWrite : TBUFFER = %X", data);
	...
	case 0x11:
		MODEM_LOG("PARSL = %d", modem_regs.reg11.PARSL);
		die("PARSL");
		break;
	...
	case 0x14:	// ABCODE
		if (data == 0x4f || data == 0x5f)
		{
			...
			//MODEM_LOG("SPX/CTL download initiated");
		...
		else if (data == 0 && module_download)
		{
			...
			//MODEM_LOG("SPX/CTL download finished CRC %02x", download_crc);
		}
		break;
	...
	case 0x1D:	// MEADDH
		...
		if (modem_regs.reg1c_1d.MEACC)
		{
			...
			{
				MODEM_LOG("DSP mem Write%s address %08x = %x", word_dspram_write ? " (w)" : "", dspram_addr, modem_regs.reg18_19);
			...
			else
			{
				...
				MODEM_LOG("DSP mem Read address %08x == %x", dspram_addr, modem_regs.reg18_19 );
			}
		}
		break;
	...
	case 0x1F:
		...
		if (modem_regs.reg1f.NEWC)
		{
			if(modem_regs.reg1a.SFRES)
			{
				...
				MODEM_LOG("Soft Reset SET && NEWC, executing reset and init");
				modem_reset(1);
			}
			else
			{
				...
				MODEM_LOG("NEWC CONF=%x", modem_regs.reg12);
			}
		}
	...
	default:
		//MODEM_LOG("ModemNormalWrite : undef %03X = %X", reg, data);
		break;
	}
	update_interrupt();
}
...
u32 ModemReadMem_A0_006(u32 addr, u32 size)
{
	...
	if (reg < 0x21)
	{
		...
		case MS_NORMAL:
			{
				...
				{
					//MODEM_LOG("SECTXB / SECTXB being read %02x", reg)
				}
				//else
				//	MODEM_LOG("Read Reg %03x = %x", reg, data);
		...
		default:
			//MODEM_LOG("Read (reset) reg %03x == %x", reg, modem_regs.ptr[reg]);
			return 0;
		}
	}

	MODEM_LOG("modem reg %03X read -- wtf is it ?",reg);
	return 0;
}
...
void ModemWriteMem_A0_006(u32 addr, u32 data, u32 size)
{
	...
	if (reg < 0x100)
	{
		MODEM_LOG("modem reg %03X write -- MODEM ID?!",reg);
		return;
	}
	...
	if (reg < 0x20)
	{
		if (state == MS_NORMAL)
			...
			MODEM_LOG("modem reg %03X write %X -- undef state?", reg, data);
		return;
	}
	if (reg == 0x20)
	{
	...
	MODEM_LOG("modem reg %03X write %X -- wtf is it?",reg,data);
}
