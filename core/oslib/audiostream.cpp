...
// Using a pointer to avoid out of order init
static std::unique_ptr<std::vector<audiobackend_t *>> audiobackends;
...
#else
  Buffer[writePtr].r = r * 2.0;
  Buffer[writePtr].l = l * 2.0;
#endif
  if (++writePtr == SAMPLE_COUNT) {
    if (audiobackend_current != nullptr) {
      if (config::UseLowPass) {
        DspDo((short *) (Buffer), SAMPLE_COUNT);
      }
