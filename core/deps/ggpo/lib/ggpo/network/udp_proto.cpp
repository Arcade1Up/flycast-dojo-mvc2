...
void UdpProtocol::SendInput(GameInput &input) {
  if (_udp) {
    if (_current_state == Running) {
       ...
       */
      _pending_output.push_back(input);
    }
    SendPendingOutput();
  }
}
...
bool UdpProtocol::OnLoopPoll(void *cookie) {
  ...
  switch (_current_state) {
    case Syncing:next_interval = (_state.sync.roundtrips_remaining == NUM_SYNC_PACKETS) ? SYNC_FIRST_RETRY_INTERVAL : SYNC_RETRY_INTERVAL;
...
void UdpProtocol::SendSyncRequest() {
  ...
  msg->verification_size = _verification.size();
  if (!_verification.empty())
    memcpy(&msg->u.sync_request.verification[0], &_verification[0], _verification.size());
  SendMsg(msg);
}
...
void UdpProtocol::LogMsg(const char *prefix, UdpMsg *msg) {
  switch (msg->hdr.type) {
    ...
    case UdpMsg::SyncReply:
      ...
      break;
    case UdpMsg::QualityReport:Log("%s quality report.\n", prefix);
      break;
    case UdpMsg::QualityReply:Log("%s quality reply.\n", prefix);
      break;
    case UdpMsg::KeepAlive:Log("%s keep alive.\n", prefix);
      break;
    case UdpMsg::Input:Log("%s game-compressed-input %d (+ %d bits).\n", prefix, msg->u.input.start_frame, msg->u.input.num_bits);
      break;
    case UdpMsg::InputAck:Log("%s input ack.\n", prefix);
      break;
    case UdpMsg::AppData:Log("%s app data (%d bytes).\n", prefix, msg->u.app_data.size);
      break;
    default:ASSERT(false && "Unknown UdpMsg type.");
  }
}
...
void UdpProtocol::SendQualityReport() {
  UdpMsg *msg = new UdpMsg(UdpMsg::QualityReport);
  msg->u.quality_report.start_time = GGPOPlatform::GetCurrentTimeMS();
  msg->u.quality_report.frame_advantage = (uint8) _local_frame_advantage;
  msg->u.quality_report.remote_ping_sum = _local_round_trip_time;
  SendMsg(msg);
}
void UdpProtocol::SendQualityReply(uint32 start_time) {
  // send a reply so the other side can compute the round trip transmit time.
  UdpMsg *msg = new UdpMsg(UdpMsg::QualityReply);
  msg->u.quality_reply.end_time = start_time;
  SendMsg(msg);
}
...
bool UdpProtocol::OnSyncRequest(UdpMsg *msg, int len) {
  ...
  if (msgVerifSize != (int) _verification.size()
      || (msgVerifSize != 0 && memcmp(&msg->u.sync_request.verification[0], &_verification[0], msgVerifSize))) {
    Log("Verification mismatch: size received %d expected %d", msgVerifSize, (int) _verification.size());
...
bool UdpProtocol::OnQualityReport(UdpMsg *msg, int len) {
  SendQualityReply(msg->u.quality_report.start_time);
  _remote_frame_advantage = msg->u.quality_report.frame_advantage;
  _remote_round_trip_time = msg->u.quality_report.remote_ping_sum;
  return true;
}

bool UdpProtocol::OnQualityReply(UdpMsg *msg, int len) {
  _local_round_trip_time = GGPOPlatform::GetCurrentTimeMS() - msg->u.quality_reply.end_time;
  return true;
}
...
void UdpProtocol::GetNetworkStats(struct GGPONetworkStats *s) {
  s->network.local_ping = _local_round_trip_time;
  s->network.remote_ping = _remote_round_trip_time;
...
void UdpProtocol::SetLocalFrameNumber(int localFrame) {
   ...
   * trip time.
   */
  int remoteFrame = _last_received_input.frame + (_local_round_trip_time * 60 / 1000);
