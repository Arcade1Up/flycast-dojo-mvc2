...
public:
   ...
   void SetVerificationData(const void *verification, int verification_size) {
	   ...
	   this->_verification.resize(verification_size);
	   memcpy(&this->_verification[0], verification, verification_size);
   }
...
protected:
   ...
   void SendQualityReply(uint32 start_time);
...
protected:
   ...
   std::queue<QueueEntry> _send_queue;
   std::mutex     _send_mutex;

   std::vector<uint8> _verification;

  /*
    * Stats
    */
   int            _local_round_trip_time{0};
   int            _remote_round_trip_time{0};
   int            _packets_sent;
   int            _bytes_sent;
   int            _kbps_sent;
   ...
   std::deque<GameInput>      _pending_output;
   ...
   std::queue<UdpProtocol::Event>         _event_queue;
};
