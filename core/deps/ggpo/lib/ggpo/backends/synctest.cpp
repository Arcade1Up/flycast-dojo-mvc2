                           ...
                           int *disconnect_flags)
{
   ...
   if (_rollingback) {
      ...
      if (_sync.frameCount() == 0 && _sync.getLastSavedFrame().buf == nullptr) {
...
{
   char filename[4096];
   snprintf(filename, ARRAY_SIZE(filename), "synclogs/state-%04d-original.log", _sync.frameCount());
   _callbacks.log_game_state(filename, (unsigned char *)info.buf, info.cbuf);

   snprintf(filename, ARRAY_SIZE(filename), "synclogs/state-%04d-replay.log", _sync.frameCount());
