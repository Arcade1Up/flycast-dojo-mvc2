...
Peer2PeerBackend::Peer2PeerBackend(GGPOSessionCallbacks *cb,
  ...
  _sync.initialize(config);

  /*
   * Initialize the UDP port
   */
  _udp.initialize(localport, &_poll, this);
  ...
  if (!_sync.rollingBack()) {
    ...
    if (!_synchronizing) {
      ...
      if (total_min_confirmed >= 0) {
        ...
        ggpoLog("setting confirmed frame in sync to %d.\n", total_min_confirmed);
  ...
  for (queue = 0; queue < _num_players; queue++) {
    ...
    for (i = 0; i < _num_players; i++) {
      ...
      if (_endpoints[i].IsRunning()) {
        ...
        ggpoLog("  endpoint %d: connected = %d, last_received = %d, queue_min_confirmed = %d.\n", i, connected, last_received, queue_min_confirmed);
      } else {
        ggpoLog("  endpoint %d: ignoring... not running.\n", i);
      }
    }
    ...
    if (queue_connected) {
      ...
      if (!_local_connect_status[queue].disconnected || _local_connect_status[queue].last_frame > queue_min_confirmed) {
        ggpoLog("disconnecting queue %d by remote request.\n", queue);
...
Peer2PeerBackend::AddLocalInput(GGPOPlayerHandle player,
  ...
  if (input.frame != GameInput::NullFrame) { // xxx: <- comment why this is the case
    ...
    ggpoLog("setting local connect status for local queue %d to %d", queue, input.frame);
  ...
  switch (evt.type) {
    case UdpProtocol::Event::Input:
      if (!_local_connect_status[queue].disconnected) {
        ...
        ggpoLog("setting remote connect status for queue %d to %d\n", queue, evt.u.input.input.frame);
        _local_connect_status[queue].last_frame = evt.u.input.input.frame;
      }
      break;

    case UdpProtocol::Event::Disconnected:DisconnectPlayer(QueueToPlayerHandle(queue));
      break;
    ...
    case UdpProtocol::Event::AppData:
      ...
      ggpoLog("calling on_message callback with %d bytes", evt.u.app_data.size);
...
Peer2PeerBackend::OnUdpProtocolEvent(UdpProtocol::Event &evt, GGPOPlayerHandle handle) {
  ...
  switch (evt.type) {
    case UdpProtocol::Event::Connected:info.code = GGPO_EVENTCODE_CONNECTED_TO_PEER;
      info.u.connected.player = handle;
      _callbacks.on_event(&info);
      break;
    case UdpProtocol::Event::Synchronizing:info.code = GGPO_EVENTCODE_SYNCHRONIZING_WITH_PEER;
      info.u.synchronizing.player = handle;
      info.u.synchronizing.count = evt.u.synchronizing.count;
      info.u.synchronizing.total = evt.u.synchronizing.total;
      _callbacks.on_event(&info);
      break;
    case UdpProtocol::Event::Synchronzied:info.code = GGPO_EVENTCODE_SYNCHRONIZED_WITH_PEER;
    ...
    case UdpProtocol::Event::NetworkInterrupted:info.code = GGPO_EVENTCODE_CONNECTION_INTERRUPTED;
    ...
    case UdpProtocol::Event::NetworkResumed:info.code = GGPO_EVENTCODE_CONNECTION_RESUMED;
...
Peer2PeerBackend::DisconnectPlayer(GGPOPlayerHandle player) {
  ...
  if (!_endpoints[queue].IsInitialized()) {
    ...
    ggpoLog("Disconnecting local player %d at frame %d by user request.\n", queue, _local_connect_status[queue].last_frame);
    for (int i = 0; i < _num_players; i++) {
      if (_endpoints[i].IsInitialized()) {
    ...
    ggpoLog("Disconnecting queue %d at frame %d by user request.\n", queue, _local_connect_status[queue].last_frame);
...
Peer2PeerBackend::DisconnectPlayerQueue(int queue, int syncto) {
  ...
  ggpoLog("Changing queue %d local connect status for last frame from %d to %d on disconnect request (current: %d).\n",
  ...
  if (syncto != GameInput::NullFrame && syncto < framecount) {
    ggpoLog("adjusting simulation to account for the fact that %d disconnected @ %d.\n", queue, syncto);
    _sync.adjustSimulation(syncto);
    ggpoLog("finished adjusting simulation.\n");
  }
...
Peer2PeerBackend::GetNetworkStats(GGPONetworkStats *stats, GGPOPlayerHandle player) {
  ...
  stats->sync.predicted_frames = _sync.predictedFrames();
