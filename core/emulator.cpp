...
void dc_reset(bool hard) {
  ...
  if (hard) _vmem_unprotect_vram(0, VRAM_SIZE);
...
static void setPlatform(int platform) {
  ...
  switch (platform) {
    case DC_PLATFORM_DREAMCAST: settings.platform.ram_size = 16 * 1024 * 1024;
      ...
      break;
    case DC_PLATFORM_NAOMI: settings.platform.ram_size = 32 * 1024 * 1024;
      ...
      break;
    case DC_PLATFORM_NAOMI2: settings.platform.ram_size = 32 * 1024 * 1024;
      settings.platform.vram_size = 16 * 1024 * 1024; // 2x16 MB VRAM, only 16 emulated
      settings.platform.aram_size = 8 * 1024 * 1024;
      settings.platform.bios_size = 2 * 1024 * 1024;
      settings.platform.flash_size = 32 * 1024;    // battery-backed ram
      break;
    case DC_PLATFORM_ATOMISWAVE: settings.platform.ram_size = 16 * 1024 * 1024;
      settings.platform.vram_size = 8 * 1024 * 1024;
      settings.platform.aram_size = 8 * 1024 * 1024;
...
void Emulator::loadGame(const char *path, LoadProgress *progress) {
  init();
  try {
    ZGS_LOG("Loading game %s", (path == nullptr ? "(nil)" : path));
    if (path != nullptr)
...
void Emulator::unloadGame() {
  ...
  if (state == Loaded || state == Error) {
    // todo: Fix dc_reset will cause the program to exit abnormally
    // dc_reset(true);
...
void Emulator::start() {
  ...
  if (config::DynarecEnabled) {
    Get_Sh4Recompiler(&sh4_cpu);
    ZGS_LOG("DYNAREC: Using Recompiler");
  } else
#endif
  {
    Get_Sh4Interpreter(&sh4_cpu);
    ZGS_LOG("DYNAREC: Using Interpreter");
  }
...
void Emulator::vblank() {
  ...
  // Time out if a frame hasn't been rendered for 20 ms
  if (sh4_sched_now64() - startTime <= 4'000'000)
    return;
  renderTimeout = true;

  sen::emulator.endOneFrame();
}
