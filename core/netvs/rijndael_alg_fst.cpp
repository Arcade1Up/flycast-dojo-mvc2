...
//////////////////////////////////////////////////////////////////////////
// 函数名：rijndaelKeySetupEnc
// 描述：加密密钥扩展函数
// 输入参数：128位，4个字的密钥cipherKey；密钥长度keyBits，这里我们用128
// 输出参数：生成的（Nr+1）个轮密钥
// 返回值：为该密钥长度决定的轮数Nr
//////////////////////////////////////////////////////////////////////////
int rijndaelKeySetupEnc(u32 rk[], const u8 cipherKey[], int keyBits) {
    int i = 0;
    u32 temp;
    rk[0] = GETU32(cipherKey     );    //将cipherKey指向的32位空间赋值给rk[0]
    rk[1] = GETU32(cipherKey +  4);     //将（cipherKey+4）指向的32位空间赋值给rk[1]
    rk[2] = GETU32(cipherKey +  8);     //将（cipherKey+8）指向的32位空间赋值给rk[2]
    rk[3] = GETU32(cipherKey + 12);     //将（cipherKey+12）指向的32位空间赋值给rk[3]
    if (keyBits == 128) {       //这是在密钥长度=128的情况下，轮密钥产生的过程 
        for (;;) {
            temp  = rk[3];
            rk[4] = rk[0] ^
                (Te4[(temp >> 16) & 0xff] & 0xff000000) ^ //再次出现手动左移后，直接查表得到结果
...
// 函数名：rijndaelEncrypt
// 描述：AES加密函数
// 输入参数：（Nr+1）个轮密钥rk（4个字）；加密轮数Nr；明文16个字节pt
// 输出参数：密文16个字节ct
// 返回值：无
//////////////////////////////////////////////////////////////////////////
void rijndaelEncrypt(const u32 rk[], int Nr, const u8 pt[16], u8 ct[16]) {
    u32 s0, s1, s2, s3, t0, t1, t2, t3;    //这8个字结构非常关键。交替存储着state的状态
#ifndef FULL_UNROLL
    int r;
#endif

    s0 = GETU32(pt     ) ^ rk[0];     //第一轮，先和轮密钥的0~4xor运算
    s1 = GETU32(pt +  4) ^ rk[1];     //得到s0~s3
    s2 = GETU32(pt +  8) ^ rk[2];
    ...
    r = Nr >> 1;         //每个循环加密2轮，所以总轮数除以2
    for (;;)
    {
        t0 =
            Te0[(s0 >> 24)       ] ^    //程序的关键在这个地方，我们手工得到行移位后的结果，直接代入经过列混淆和s变换后的表中得到结果
            Te1[(s1 >> 16) & 0xff] ^
            Te2[(s2 >>  8) & 0xff] ^
            Te3[(s3      ) & 0xff] ^
            rk[4];        //每一轮的最后要异或上该轮的密钥！！！
        t1 =
            Te0[(s1 >> 24)       ] ^
            Te1[(s2 >> 16) & 0xff] ^
            Te2[(s3 >>  8) & 0xff] ^
