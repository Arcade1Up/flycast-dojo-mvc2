...
void LowPass2::Filter(short *Buff, int Tam) {
  int a;
  int Tmp, Tmp2;

  for (a = 0; a < Tam * 2; a += 2) {
    Tmp =
        (b0 * Buff[a] + b1 * i1 + b2 * i2 - a1 * o1 - a2 * o2) / (1 << FixBits);

    Tmp2 = (b0b * Buff[a] + b1b * i1 + b2b * i2 - a1b * o1b - a2b * o2b) /
           (1 << FixBits);
