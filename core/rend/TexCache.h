...
public:
	...
	void init(u32 width, u32 height, pixel_type *buffer)
	{
		deinit();
		dontFreeBuffer = true;
		p_buffer_start = p_current_line = p_current_pixel = p_current_mipmap = buffer;
		this->pixels_per_line = width;
	}
	...
	Texture *getTextureCacheData(TSP tsp, TCW tcw)
	{
		...
		if (tcw.PixelFmt == PixelPal4 || tcw.PixelFmt == PixelPal8)
		{
			if (BaseTextureCacheData::IsGpuHandledPaletted(tsp, tcw))
				...
				key |= (u64)(tcw.full & 0xF81FFFFF) << 32;
			else
