...
static void gles_term() {
  TexCache.FreePreloadedTextures();
...
#ifdef DUMP_SHADER_BINARIES
    GLint binaryLen;
    glGetProgramiv(shader->program, GL_PROGRAM_BINARY_LENGTH, &binaryLen);
    if (binaryLen > 0) {
      std::unique_ptr<u8> data = std::unique_ptr<u8>(new u8[binaryLen]);
      GLenum format;
      glGetProgramBinary(shader->program, binaryLen, nullptr, &format, data.get());
      char fname[64];
      sprintf(fname, "%x.shaderbin", rv);
      std::string path = hostfs::getShaderCachePath(fname);
      FILE *f = nowide::fopen(path.c_str(), "wb");
      if (f) {
        fwrite(&format, sizeof(format), 1, f);
        fwrite(data.get(), 1, binaryLen, f);
        fclose(f);
      }
    }
#endif
...
static void getPipelineUniforms(PipelineShader* s)
{
...
void gl_DebugOutput(GLenum source,
  ...
  switch (severity) {
    ...
    case GL_DEBUG_SEVERITY_LOW: DEBUG_LOG(RENDERER, "opengl:[%d] %s", id, message);
      break;
    case GL_DEBUG_SEVERITY_MEDIUM: INFO_LOG(RENDERER, "opengl:[%d] %s", id, message);
      break;
    case GL_DEBUG_SEVERITY_HIGH: WARN_LOG(RENDERER, "opengl:[%d] %s", id, message);
      break;
  }
}
#endif

static void loadShaderBinaries()
{
	std::string dirPath = hostfs::getShaderCachePath("");
	DIR *dir = flycast::opendir(dirPath.c_str());
	if (dir == nullptr)
		return;
	...
	while (true)
	{
		...
		if (entry == nullptr)
			break;
		if (get_file_extension(entry->d_name) != "shaderbin")
			continue;
		FILE *f = nowide::fopen(hostfs::getShaderCachePath(entry->d_name).c_str(), "rb");
		if (f)
		{
			GLenum format;
			...
			{
				fseek(f, 0, SEEK_END);
				long size = ftell(f) - sizeof(format);
				fseek(f, sizeof(format), SEEK_SET);
				std::unique_ptr<u8> data = std::unique_ptr<u8>(new u8[size]);
				if (fread(data.get(), 1, size, f) == size)
				{
					GLuint program = glCreateProgram();
					glProgramBinary(program, format, data.get(), size);
					if (glIsProgram(program))
					{
						long shaderId = strtol(entry->d_name, nullptr, 16);
						PipelineShader *shader = &gl.shaders[shaderId];
						shader->program = program;
						shader->divPosZ = shaderId & 1;
						shader->naomi2 = (shaderId >> 1) & 1;
						shader->palette = (shaderId >> 2) & 1;
						shader->trilinear = (shaderId >> 3) & 1;
						shader->fog_clamping = (shaderId >> 4) & 1;
						shader->pp_BumpMap = (shaderId >> 5) & 1;
						shader->pp_Gouraud = (shaderId >> 6) & 1;
						shader->pp_FogCtrl = (shaderId >> 7) & 3;
						shader->pp_Offset = (shaderId >> 9) & 1;
						shader->pp_ShadInstr = (shaderId >> 10) & 3;
						shader->pp_IgnoreTexA = (shaderId >> 12) & 1;
						shader->pp_UseAlpha = (shaderId >> 13) & 1;
						shader->pp_Texture = (shaderId >> 14) & 1;
						shader->cp_AlphaTest = (shaderId >> 15) & 1;
						shader->pp_InsideClipping = (shaderId >> 16) & 1;
						glcache.UseProgram(program);
						getPipelineUniforms(shader);
					}
				}
			}
			fclose(f);
...
static void precompileMvsC2Shaders()
{
  GetProgram(false, false, false, false, true,  1, false, 2, true,  false, false, false, false, false);
  GetProgram(false, false, true,  true,  false, 3, false, 0, true,  false, false, false, false, false);
  GetProgram(false, false, true,  false, true,  1, true,  0, true,  false, false, false, false, false);
  GetProgram(false, false, false, true,  false, 3, false, 0, true,  false, false, false, false, false);
  GetProgram(false, false, true,  false, true,  1, true,  0, false, false, false, false, false, false);
  GetProgram(false, false, true,  false, false, 1, true,  0, false, false, false, false, false, false);
  GetProgram(false, false, true,  false, false, 1, true,  0, true,  false, false, false, false, false);
  GetProgram(false, false, true,  true,  true,  3, true,  0, true,  false, false, false, false, false);
  GetProgram(false, false, true,  false, true,  1, true,  2, true,  false, false, false, false, false);
  GetProgram(false, false, true,  false, true,  1, true,  2, false, false, false, false, false, false);
  GetProgram(false, false, true,  false, false, 1, true,  2, true,  false, false, false, false, false);
  GetProgram(false, false, true,  false, false, 1, true,  2, false, false, false, false, false, false);
  GetProgram(false, false, true,  true,  false, 3, false, 2, false, false, false, false, false, false);
  GetProgram(false, false, true,  true,  false, 3, true,  2, true,  false, false, false, false, false);
  GetProgram(false, false, true,  true,  true,  3, true,  2, true,  false, false, false, false, false);
  GetProgram(false, false, true,  true,  false, 3, true,  0, false, false, false, false, false, false);
  GetProgram(false, false, true,  true,  false, 3, true,  2, false, false, false, false, false, false);
  GetProgram(false, false, true,  true,  false, 3, false, 2, false, false, false, false, true,  false);
  GetProgram(false, false, true,  true,  true,  3, true,  2, false, false, false, false, false, false);
  GetProgram(true,  false, true,  false, false, 1, true,  0, true,  false, false, false, false, false);
  GetProgram(false, false, true,  false, false, 0, true,  2, true,  false, false, false, false, false);
  GetProgram(false, false, true,  true,  true,  3, true,  0, false, false, false, false, false, false);
  GetProgram(false, false, true,  true,  false, 3, true,  0, true,  false, false, false, false, false);
}
...
bool gles_init() {
  ...
  loadShaderBinaries();
  precompileMvsC2Shaders();
  TexCache.PreloadTextures();
