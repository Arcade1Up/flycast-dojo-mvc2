...
{
public:
	TextureCacheData(TSP tsp, TCW tcw) : BaseTextureCacheData(tsp, tcw), texID(glcache.GenTexture()), sharedID(false) {
	}
	TextureCacheData(TextureCacheData&& other) : BaseTextureCacheData(std::move(other)) {
		std::swap(texID, other.texID);
		std::swap(sharedID, other.sharedID);
	}
...
public:
	...
	void FreePreloadedTextures();
	std::unordered_map<u32, GLuint> preloadedTextures; // indexed by hash
...
struct OpenGLRenderer : Renderer
{
	...
	BaseTextureCacheData *GetTexture(TSP tsp, TCW tcw, PolyParam *pp) override
	{
		return gl_GetTexture(tsp, tcw, pp);
	}
