...
static void SetGPState(const PolyParam* gp)
{
	...
	if (CurrentShader->pp_Texture)
	{
        ...
        if (texture != nullptr)
        {
            ...
            glBindSampler(0, texSamplers[0]);
            SetTextureRepeatMode(0, GL_TEXTURE_WRAP_S, tsp.ClampU, tsp.FlipU);
            SetTextureRepeatMode(0, GL_TEXTURE_WRAP_T, tsp.ClampV, tsp.FlipV);
            ...
            if (nearest_filter)
            {
                ...
                glSamplerParameteri(texSamplers[0], GL_TEXTURE_MIN_FILTER, mipmapped ? GL_NEAREST_MIPMAP_LINEAR : GL_NEAREST);
                glSamplerParameteri(texSamplers[0], GL_TEXTURE_MAG_FILTER, GL_NEAREST);
            }
            else
            {
                //bilinear filtering
                //PowerVR supports also trilinear via two passes, but we ignore that for now
                glSamplerParameteri(texSamplers[0], GL_TEXTURE_MIN_FILTER, mipmapped ? GL_LINEAR_MIPMAP_LINEAR : GL_LINEAR);
                glSamplerParameteri(texSamplers[0], GL_TEXTURE_MAG_FILTER, GL_LINEAR);
            }

            if (mipmapped)
                glSamplerParameterf(texSamplers[0], GL_TEXTURE_LOD_BIAS, D_Adjust_LoD_Bias[tsp.MipMapD]);
