...
u32 GamepadDevice::Opposite_gd(u32 dir)
{
	switch(dir)
	{
		case DC_DPAD_LEFT:
...
bool GamepadDevice::CorrectDiag(std::tuple<u32, u32> diag, int port)
{
	...
	{
		if ((~kcode_prev[port] & Opposite_gd(x)) && (~kcode_prev[port] & y))
		{
			NOTICE_LOG(INPUT, "CORRECTED DIAGONAL *%s %s to %s %s", DirStr(Opposite_gd(x)).c_str(), DirStr(y).c_str(), DirStr(x).c_str(), DirStr(y).c_str());
			kcode[port] |= x;
			kcode[port] |= Opposite_gd(x);
			kcode_next[port] = ~(x | y);

			return true;
		}
		else if ((~kcode_prev[port] & x) && (~kcode_prev[port] & Opposite_gd(y)))
		{
			NOTICE_LOG(INPUT, "CORRECTED DIAGONAL %s *%s to %s %s", DirStr(x).c_str(), DirStr(Opposite_gd(y)).c_str(), DirStr(x).c_str(), DirStr(y).c_str());
			kcode[port] |= y;
			kcode[port] |= Opposite_gd(y);
			kcode_next[port] = ~(x | y);
