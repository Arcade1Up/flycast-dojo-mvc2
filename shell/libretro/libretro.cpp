...
static void update_variables(bool first_startup) {
  ...
  if (var.value != nullptr && !strcmp(var.value, "per-pixel (accurate)")) {
    switch (config::RendererType) {
      case RenderType::Vulkan: config::RendererType = RenderType::Vulkan_OIT;
        break;
      case RenderType::DirectX11: config::RendererType = RenderType::DirectX11_OIT;
        break;
      case RenderType::OpenGL: config::RendererType = RenderType::OpenGL_OIT;
        break;
      default: break;
    }
    ...
    switch (config::RendererType) {
      case RenderType::Vulkan_OIT: config::RendererType = RenderType::Vulkan;
        break;
      case RenderType::DirectX11_OIT: config::RendererType = RenderType::DirectX11;
        break;
      case RenderType::OpenGL_OIT: config::RendererType = RenderType::OpenGL;
        break;
      default: break;
    }
...
static uint32_t map_gamepad_button(unsigned device, unsigned id) {
  ...
  switch (settings.platform.system) {
    ...
    case DC_PLATFORM_DEV_UNIT:
      switch (device) {
        case RETRO_DEVICE_JOYPAD: joymap = dc_joymap;
          joymap_size = ARRAY_SIZE(dc_joymap);
          break;
        case RETRO_DEVICE_LIGHTGUN: joymap = dc_lg_joymap;
    ...
    case DC_PLATFORM_NAOMI2:
      switch (device) {
        case RETRO_DEVICE_JOYPAD: joymap = nao_joymap;
          joymap_size = ARRAY_SIZE(nao_joymap);
          break;
        case RETRO_DEVICE_LIGHTGUN: joymap = nao_lg_joymap;
    ...
    case DC_PLATFORM_ATOMISWAVE:
      switch (device) {
        case RETRO_DEVICE_JOYPAD: joymap = aw_joymap;
          joymap_size = ARRAY_SIZE(aw_joymap);
          break;
        case RETRO_DEVICE_LIGHTGUN: joymap = aw_lg_joymap;
  ...
  if (settings.platform.isArcade()) {
    ...
    for (unsigned i = 0; i < MAPLE_PORTS; i++) {
      switch (config::MapleMainDevices[i]) {
        case MDT_LightGun: name = get_button_name(RETRO_DEVICE_LIGHTGUN, RETRO_DEVICE_ID_LIGHTGUN_DPAD_LEFT, "D-Pad Left");
          if (name != NULL)
        ...
        case MDT_SegaController: name = get_button_name(RETRO_DEVICE_JOYPAD, RETRO_DEVICE_ID_JOYPAD_LEFT, "D-Pad Left");
          if (name != NULL)
    ...
    for (unsigned i = 0; i < MAPLE_PORTS; i++) {
      switch (config::MapleMainDevices[i]) {
        case MDT_SegaController: desc[descriptor_index++] = {i, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_LEFT, "D-Pad Left"};
        ...
        case MDT_TwinStick: desc[descriptor_index++] = {i, RETRO_DEVICE_TWINSTICK, 0, RETRO_DEVICE_ID_JOYPAD_LEFT, "L-Stick Left"};
        ...
        case MDT_AsciiStick: desc[descriptor_index++] = {i, RETRO_DEVICE_TWINSTICK, 0, RETRO_DEVICE_ID_JOYPAD_LEFT, "Stick Left"};
        ...
        case MDT_LightGun: desc[descriptor_index++] = {i, RETRO_DEVICE_LIGHTGUN, 0, RETRO_DEVICE_ID_LIGHTGUN_DPAD_LEFT, "D-Pad Left"};
...
void retro_set_controller_port_device(unsigned in_port, unsigned device) {
  if (device_type[in_port] != (int) device && in_port < MAPLE_PORTS) {
    ...
    switch (device) {
      case RETRO_DEVICE_JOYPAD: config::MapleMainDevices[in_port] = MDT_SegaController;
        if (settings.platform.isConsole()) {
      ...
      case RETRO_DEVICE_TWINSTICK_SATURN: config::MapleMainDevices[in_port] = MDT_TwinStick;
        if (settings.platform.isConsole()) {
        ...
        break;
      case RETRO_DEVICE_ASCIISTICK: config::MapleMainDevices[in_port] = MDT_AsciiStick;
        if (settings.platform.isConsole()) {
          config::MapleExpansionDevices[in_port][0] = enable_purupuru ? MDT_PurupuruPack : MDT_SegaVMU;
          config::MapleExpansionDevices[in_port][1] = MDT_None;
        }
        break;
      case RETRO_DEVICE_KEYBOARD: config::MapleMainDevices[in_port] = MDT_Keyboard;
        if (settings.platform.isConsole()) {
          config::MapleExpansionDevices[in_port][0] = MDT_None;
          config::MapleExpansionDevices[in_port][1] = MDT_None;
        }
        break;
      case RETRO_DEVICE_MOUSE: config::MapleMainDevices[in_port] = MDT_Mouse;
        if (settings.platform.isConsole()) {
          config::MapleExpansionDevices[in_port][0] = MDT_None;
          config::MapleExpansionDevices[in_port][1] = MDT_None;
        }
        break;
      case RETRO_DEVICE_LIGHTGUN: config::MapleMainDevices[in_port] = MDT_LightGun;
        if (settings.platform.isConsole()) {
          config::MapleExpansionDevices[in_port][0] = enable_purupuru ? MDT_PurupuruPack : MDT_SegaVMU;
          config::MapleExpansionDevices[in_port][1] = MDT_None;
        }
        break;
      default: config::MapleMainDevices[in_port] = MDT_None;
        if (settings.platform.isConsole()) {
          config::MapleExpansionDevices[in_port][0] = MDT_None;
          config::MapleExpansionDevices[in_port][1] = MDT_None;
        }
      ...
      for (int id = RETRO_DEVICE_ID_JOYPAD_B; id <= RETRO_DEVICE_ID_JOYPAD_R3; ++id) {
        switch (id) {
          ...
          case RETRO_DEVICE_ID_JOYPAD_L:
            ...
            break;
          default: setDeviceButtonStateFromBitmap(ret, port, RETRO_DEVICE_JOYPAD, id);
            break;
        }
      }
      //
...
static void UpdateInputState(u32 port) {
  ...
  switch (config::MapleMainDevices[port]) {
    ...
    case MDT_Mouse: updateMouseState(port);
      break;
